document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- !!!!!!!!!!!!!!!!!!!!!!!!! ---
    //
    // YOUR WEB APP URL IS ADDED HERE
    //
    ...
    function handleLogout() {
        emailInput.value = 'mridulbaishya216@gmail.com';
        passwordInput.value = 'Jjm@mug25';
        captchaInput.value = '';
        generateCaptcha();
        showPage('page-login');
    }
    ...
    function loadDashboardData() {
        ...
        document.getElementById('stat-monthly-collection').textContent = `₹${monthlyTotal.toLocaleString('en-IN')}`;

        const totalCollection = transactions.reduce((sum, t) => sum + parseFloat(t.amount), 0);
        document.getElementById('stat-total-collection').textContent = `₹${totalCollection.toLocaleString('en-IN')}`;
        ...
        document.getElementById('stat-dues-pending').textContent = `₹${totalDues.toLocaleString('en-IN')}`;
    }
    ...
    function loadEditableMemberList(filter = '') {
        ...
        if (filteredMembers.length === 0) {
            memberListEdit.innerHTML = '<p>No members found.</p>';
            return;
        }
    ...
    function loadFeesCollectionList(filter = '') {
        ...
        if (filteredMembers.length === 0) {
            feesMemberList.innerHTML = '<p>No members found.</p>';
            return;
        }
